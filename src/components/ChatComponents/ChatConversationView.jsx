...
function getMessageAuthor(message, chat) {
  if (message.is_admin) {
    return {
      label: message?.sender?.full_name || "Admin",
      role: "admin",
    };
  }
  ...
  if (message.sender_id === chat?.buyer_id) {
    return {
      label: chat?.buyer?.full_name || message?.sender?.full_name || "Buyer",
      role: chat?.buyer?.role || "buyer",
    };
  }

  if (message.sender_id === chat?.seller_id) {
    return {
      label: chat?.seller?.full_name || message?.sender?.full_name || "Seller",
      role: chat?.seller?.role || "seller",
    };
  }

  return {
    label: message?.sender?.full_name || "User",
    role: message?.sender?.role || "buyer",
  };
}

export default function AdminChatConversationView({
  chat,
  currentUserId,
  viewerRole,
  messages,
  ...
  const viewerPalette = getRolePalette(viewerRole);
  const viewerRoleLabel = formatRole(viewerRole);
  const otherUser =
  ...
  useLayoutEffect(() => {
    ...
    const isFirst =
      prevCountRef.current === 0 || prevCountRef.current > messages.length;
    prevCountRef.current = messages.length;

    const behavior = isFirst || loading ? "auto" : "smooth";
    let rafOne = 0;
    let rafTwo = 0;
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            {chat?.listing?.title?.charAt(0)?.toUpperCase() || "C"}
          </Avatar>
          ...
          <Chip
            size="small"
            label={viewerRoleLabel}
            sx={{
              fontWeight: 700,
              backgroundColor: viewerPalette.tint,
          ...
          {loading ? (
            ...
            >
              ...
              <Typography variant="body2" sx={{ color: "#64748b" }}>
                Send the first admin message in this conversation.
              </Typography>
            </Paper>
          ) : (
            <Stack spacing={1}>
              {messages.map((message) => {
                ...
                const author = getMessageAuthor(message, chat);
                const palette = getRolePalette(author.role);
                const senderLabel = isMine
                  ? `You • ${viewerRoleLabel}`
                  : `${author.label} • ${formatRole(author.role)}`;
                    ...
                    <Paper
                      ...
                      sx={{
                        ...
                        borderRadius: isMine
                          ...
                          : "16px 16px 16px 6px",
                        background: isMine
                          ? `linear-gradient(135deg, ${viewerPalette.accent} 0%, ${viewerPalette.accentSoft} 100%)`
                          : palette.tint,
                        color: isMine ? "#ffffff" : "#0f172a",
                        border: isMine ? "none" : `1px solid ${palette.border}`,
                        borderLeft: isMine
                          ? "none"
                          : `4px solid ${palette.accent}`,
                        boxShadow: isMine
                          ? `0 10px 20px ${viewerPalette.accent}45`
        ...
        >
          ...
          <Box sx={{ display: "flex", gap: 1, alignItems: "flex-end", pt: 1 }}>
            ...
            <IconButton
              ...
              disabled={sending}
              sx={{
                background: `linear-gradient(135deg, ${viewerPalette.accent} 0%, ${viewerPalette.accentSoft} 100%)`,
                color: "#ffffff",
                borderRadius: 2.5,
                "&:hover": { opacity: 0.95 },
                "&.Mui-disabled": {
                  backgroundColor: viewerPalette.border,
                  color: "#ffffff",
                },
              }}
            >
